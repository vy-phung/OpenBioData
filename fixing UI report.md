...
## Follow-up: third landmine found — benchmark script bypasses the known-sample cache entirely (2026-07-07)

Found while running the 21-accession benchmark suite (`benchmark_outputs/`), not while chasing the SSE freeze — flagged here only, not fixed, since it's the same "unaudited request-path call site" family as the two landmines above.

`_cache_get`/`_cache_save` (`api.py:161`, `api.py:174` — the `KnownCachedSamples` Google Sheet read/write) live entirely inside `api.py`'s `/analyze` SSE-streaming endpoint generator: `_cache_get` is called on `resolved_dict` right before the rich pipeline runs (`api.py:1358`), and `_cache_save` is fired per completed sample after each `__partial_acc__` message (`api.py:1492`). Both are gated on `os.environ.get("GCP_CREDS_JSON")`.

`benchmark_outputs/scripts/pipeline_runner.py` calls `additional_pipeline.pipeline_with_gemini()` directly — the same function the endpoint calls — but imports nothing else from `api.py` except `_extract_text_from_upload`. It never touches `_cache_get`/`_cache_save`, so **no benchmark row reads from or writes to `KnownCachedSamples`, regardless of whether `GCP_CREDS_JSON` is set** (it is set in this environment; that's not the gap). Every accession run through the benchmark is a fresh, full-cost extraction, and none of those results become available as cache hits later.

This should be wired in — most likely by moving the cache read/write closer to `pipeline_with_gemini` itself (so both the endpoint and any direct caller like the benchmark runner get it for free), or by wrapping `pipeline_runner.run_group` with the same before/after cache calls the endpoint uses — **before customers start manually re-testing the same accessions in the UI**, so their repeat clicks get cache hits instead of paying fresh LLM/NCBI cost every time. Not scoped or implemented here; benchmark results in this run are all reported as "freshly computed (no cache path wired into benchmark script)" rather than a true fresh/cache distinction, since none could be a hit under the current wiring.
