# How OpenBioData Works — A Guide for Researchers

This document explains how the tool retrieves, infers, and scores biological sample metadata — written so you can evaluate whether the system is credible and reliable enough for your research.
...
│  abstract. Also searches Serper/Google for the sample isolate   │
│  name + accession to find any citing papers.                    │
└────────────────────────┬────────────────────────────────────────┘
                         │
┌────────────────────────▼────────────────────────────────────────┐
...
The confidence score (0–100) is calculated in `confidence_score.py` using four independent signals. The same logic applies to every metadata field.
...
This signal answers: *does the official database record back up what was found?*
...
This signal penalizes cases where the LLM extracted a different answer than the database's own field. Conflicts are flagged with a `##` marker in the output.
...
More independent sources confirming the same value increases confidence.
...
A sample (`SAMN00000001`) where:
- NCBI has a country field: **+20** (geo only)
- LLM predicted the same country as NCBI: **+20** (match)
- One linked publication found: **+10** (one pub)
- All requested fields were filled: no penalty

**Total: 50 → Medium (🟡)**

The same sample, but the paper also mentions the accession ID by name:
- Steps above: +30 (geo + pubmed) + 20 (match) + 10 (one pub) = **60 → Medium (🟡)**

With two linked publications: +30 + 20 + 20 = **70 → High (🟢)**

---

## What LLM Is Used and Why?

The tool uses **Claude Haiku** (Anthropic) as the primary LLM, with **Gemini 2.5 Flash-Lite** as a fallback. These are chosen because:

- They are fast and cost-efficient for processing dozens of samples
- They handle long-context inputs (full papers, supplementary tables)
- They follow structured output formats reliably (JSON with field/explanation pairs)

The LLM is used **only for reading and extracting** — it does not invent information. If no relevant text is found in the source documents, the output is `unknown`.
